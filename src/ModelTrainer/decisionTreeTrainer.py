...
def training_meta_decision_trees(settings_file_path, training_set, testing_set, seed, kFold =5, ):
    ...
    for target_column in META_LEANER_TARGET_COLUMNS:
        ...
        stats, path_to_model = train_meta_decision_tree(settings[target_column],
...
def train_meta_decision_tree(params,
    ...
    if kFold == 0:
        ...
        decision_trees_stats.add_model(tree)
    else:
    ...
    if target_column != 'na':
        folder_path = f"{MODEL_PATH}DecisionTrees\\{datetime.now().strftime("%Y%m%d_%H")}"
        folder_maker(folder_path)
        path_to_model = f"{folder_path}\\{target_column}.pkl"
        joblib.dump(decision_trees_stats.get_best_model(), path_to_model)

    return decision_trees_stats, path_to_model